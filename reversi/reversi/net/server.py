...
from reversi.net.protocol import send_message
...
class ReversiServer:
    ...
    def _handle_move(self, client: _ConnectedClient, payload: dict[str, Any]) -> bool:
        ...
        if self._state.skipped_turn:
            skipped_name = self._player_name(self._state.current_player.other())
            self._last_message += f" {skipped_name} has no moves, skipped."
        ...
        if self._state.finished:
            ...
            if winner is None:
                ...
                black, white = self._state.get_scores()
                finished_message = f"Round {self._round_number} ended in a draw (● {black} ○ {white})."
            else:
                key = "black_wins" if winner is Player.BLACK else "white_wins"
                self._scoreboard[key] += 1
                black, white = self._state.get_scores()
                finished_message = f"Round {self._round_number}: {winner.label} wins (● {black} ○ {white})."
    ...
    def _handle_ready(self, client: _ConnectedClient) -> bool:
        ...
        if self._all_ready():
            ...
            self._phase = "in_game"
            black_name = self._player_name(Player.BLACK)
            self._last_message = f"Round {self._round_number} started. {black_name} (●) to move."
        self._broadcast_room_state()
        return False
